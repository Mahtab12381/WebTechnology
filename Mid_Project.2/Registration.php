...
<body>
	...
	if ($_SERVER['REQUEST_METHOD'] === "POST") {
		...
		if($errorcount==0){
			if(filesize("data.json")<=0){
			$arr = array(array('firstname' => $First_Name, 'lastname' => $Last_Name ,'gender' => $Gender,'email'=> $Email,'mobile_no'=>$Mobile_no,'address'=> $SHR,'country'=>$Country,'password'=>$Password,'username'=>$Username,'image'=>$Image,'bio'=>$BIO,'dob'=>$DOB,'nationality'=>$Nationality,'blood'=>$Blood,'nid'=>$NID));
			$f = fopen("data.json", "a");
			fwrite($f, json_encode($arr));
			fclose($f);
			}
			else if(filesize("data.json")>0){
			$arr2 = array('firstname' => $First_Name, 'lastname' => $Last_Name ,'gender' => $Gender,'email'=> $Email,'mobile_no'=>$Mobile_no,'address'=> $SHR,'country'=>$Country,'password'=>$Password,'username'=>$Username,'image'=>$Image,'bio'=>$BIO,'dob'=>$DOB,'nationality'=>$Nationality,'blood'=>$Blood,'nid'=>$NID);
			$f = fopen("data.json", 'r');
			$s = fread($f, filesize("data.json"));
			$data = json_decode($s);
